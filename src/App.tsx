...
function App() {
  ...
  const loadData = async () => {
    try {
      ...
      const message =
        err instanceof APIError ? err.message : "Fehler beim Laden der Daten";
      setError(message);
    } finally {
      setLoading(false);
    }
  ...
  const handleCrawl = async (websiteId: string) => {
    try {
      ...
      const message =
        err instanceof Error ? err.message : "Fehler beim Crawl der Website";
      setError(message);
    } finally {
      setCrawlingId(null);
    }
      ...
      <main className="max-w-6xl mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-1">
            <AddWebsiteForm onSubmit={handleAddWebsite} loading={loading} />
          </div>
          ...
          <div className="lg:col-span-2">
            {loading ? (
              <div className="bg-white p-8 rounded-lg shadow-md text-center">
                ...
                <p className="text-gray-600">Websites werden geladen...</p>
              </div>
            ) : websites.length === 0 ? (
              <div className="bg-white p-8 rounded-lg shadow-md text-center">
                <p className="text-gray-600">
                  Noch keine Websites hinzugefügt. Starten Sie mit einer neuen
                  Website!
                </p>
              </div>
            ) : (
