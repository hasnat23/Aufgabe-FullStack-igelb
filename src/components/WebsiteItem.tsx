    ...
    <div className="bg-white border border-gray-200 rounded-lg shadow-sm hover:shadow-md transition">
      <div className="p-4">
        ...
        {lastChange && (
          <div className="mt-3 pt-3 border-t border-gray-100">
            <p className="text-sm text-gray-600">
              <span className="font-medium">Last scan:</span> {new Date(lastChange.data?.timestamp).toLocaleString()}
            </p>
            <p className="text-sm text-gray-600 mt-1">
              <span className="font-medium">Status:</span>{' '}
              <span className={lastChange.data?.similarity ?? 1 > 0.95 ? 'text-green-600' : 'text-orange-600'}>
                {lastChange.data?.similarity ? `${(lastChange.data.similarity * 100).toFixed(0)}% similar` : 'Changes detected'}
              </span>
            </p>
          </div>
        )}
        ...
        {changes.length > 0 && (
          ...
          >
            <span>View History ({changes.length})</span>
      ...
      {expanded && changes.length > 0 && (
        <div className="border-t border-gray-200 bg-gray-50 p-4">
          <div className="space-y-3 max-h-96 overflow-y-auto">
            {changes.map((change) => (
              <div key={change.id} className="bg-white p-3 rounded border border-gray-200 text-sm">
                <p className="text-gray-500 mb-2">
                  {new Date(change.data?.timestamp).toLocaleString()}
                </p>
                <p className="text-gray-700 whitespace-pre-wrap max-h-48 overflow-y-auto">
                  {change.data?.changes || 'No changes description available'}
                </p>
              </div>
            ))}
          </div>
